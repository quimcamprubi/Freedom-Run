...
public class AIPatrol : MonoBehaviour
{
    ...
    {
        var playerHitFront = Physics2D.Raycast(transform.position, direction, playerDetectionRange,
            LayerMask.GetMask("Player"));
        var platformHitFront = Physics2D.Raycast(transform.position, direction, playerDetectionRange,
            LayerMask.GetMask("Platforms"));
        var playerHitBack = Physics2D.Raycast(transform.position, -direction, 2,
            LayerMask.GetMask("Player"));
        if (playerHitFront.collider != null && platformHitFront.collider != null || playerHitBack.collider != null)
            isDetectedPlayer =
                Math.Abs(playerHitFront.point.x - enemyPosition.x) <
                Math.Abs(platformHitFront.point.x - enemyPosition.x) || playerHitBack.collider;
        else
